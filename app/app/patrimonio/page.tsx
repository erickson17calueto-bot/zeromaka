...
export default function PatrimonioPage() {
  ...
  return <div className="max-w-6xl mx-auto space-y-6"><header className="flex items-end justify-between flex-wrap gap-3"><div><div className="flex items-center gap-2 text-maka-400 text-sm font-semibold"><Wrench size={17} /> Património e estrutura</div><h1 className="h-page mt-1">Ativos e centros de custo</h1><p className="text-sm text-ink-400 mt-1">Organiza equipamentos, acompanha valor líquido e separa responsabilidades.</p></div><button className="btn-ghost" onClick={load}><RefreshCw size={15} /> Atualizar</button></header>

    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3"><div className="card p-4"><div className="text-[12.5px] text-ink-400 font-medium">Ativos visíveis</div><div className="text-2xl font-semibold tracking-tight tabular-nums mt-1">{visibleAssets.length}</div></div><div className="card p-4"><div className="text-[12.5px] text-ink-400 font-medium">Custo de aquisição</div><div className="text-2xl font-semibold tracking-tight tabular-nums mt-1">{fmtKz(totalCost)}</div></div><div className="card p-4"><div className="text-[12.5px] text-ink-400 font-medium">Valor líquido estimado</div><div className="text-2xl font-semibold tracking-tight tabular-nums mt-1 text-maka-400">{fmtKz(totalNet)}</div></div></div>
